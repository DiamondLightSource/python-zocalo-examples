...
class SimpleService(CommonService):
    """
    Service which takes a list of numbers, sorts them, puts the max, min and sum on outputs.
    Also outputs to positive output if the sum is greater than 0
    """

    # Human readable service name
    _service_name = "Conditional Service"

    # Logger name
    _logger_name = "examples.conditionalservice"

    def initializing(self):
        """Subscribe to the conditionalservice.submission queue. Received messages must be
       acknowledged."""
        self.log.info("Conditional Service starting")
    ...
    def process(self, rw, header, message):
        ...
        self.log.info(
            f"Running Conditional Service at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        try:
            ...
            commands = rw.recipe_step["parameters"]["numbers"]
